...
public class PhieuThueDAO {
    ...
    public boolean thanhToan(int maPhieu, String maTB) {
        String sqlUpdatePhieu = 
            "UPDATE PhieuThue p JOIN ChiTietThue ct ON p.MaPhieu = ct.MaPhieu " +
            "SET p.NgayThucTeTra = NOW(), p.TrangThai = 'Đã thanh toán', " +
            "p.TongTien = (CASE " +
            "   WHEN ct.HinhThucThue = 'gio' THEN GREATEST(TIMESTAMPDIFF(HOUR, p.NgayThue, NOW()), 1) * ct.GiaThue * ct.ThoiHan " +
            "   WHEN ct.HinhThucThue = 'ngay' THEN GREATEST(TIMESTAMPDIFF(DAY, p.NgayThue, NOW()), 1) * ct.GiaThue * ct.ThoiHan " +
            "   WHEN ct.HinhThucThue = 'thang' THEN GREATEST(TIMESTAMPDIFF(MONTH, p.NgayThue, NOW()), 1) * ct.GiaThue * ct.ThoiHan " +
            "   ELSE 0 END) + IFNULL(p.TongTienPhat, 0) - IFNULL(p.TienDatCoc, 0) " +
            "WHERE p.MaPhieu = ? AND ct.MaTB = ?";

        String sqlUpdateTB = "UPDATE ThietBi SET TrangThai = 'Trống' WHERE MaTB = ?";

        try (Connection con = Database.getConnection()) {
            con.setAutoCommit(false);
            try (PreparedStatement ps1 = con.prepareStatement(sqlUpdatePhieu);
                 PreparedStatement ps2 = con.prepareStatement(sqlUpdateTB)) {
                
                ps1.setInt(1, maPhieu);
                ps1.setString(2, maTB);
                ps1.executeUpdate();

                ps2.setString(1, maTB);
                ps2.executeUpdate();
    ...
    public long getDonGiaPhat(String hinhThuc) {
        String sql = "SELECT MucPhat FROM QuyTacPhat WHERE HinhThuc = ?";
        try (Connection con = Database.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, hinhThuc);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getLong("MucPhat");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
